 ...
 * 转换器，在行前添加与上一行的间隔时间
 */
fun C_AddTimeDiffHTMLAndOutToMultiFiles(sc: SplitConv, type: String, lines: Iterator<Line>) {
    var prevLine: Line? = null
    ...
    for (line in lines) {
        ...
        // 输出行
        sc.out(line.compute { "'${prefix} ${it.replace("'", "\\'")}\'," }, type + ".html")

        prevLine = line
    }
}

fun loadBeforeHtml(writer: Writer) {
    val resourceAsStream = Thread.currentThread().contextClassLoader.getResourceAsStream("html_pre")
    val reader = InputStreamReader(resourceAsStream!!)
    writer.write(reader.readText())
}

fun loadEndHtml(writer: Writer) {
    val resourceAsStream = Thread.currentThread().contextClassLoader.getResourceAsStream("html_post")
    val reader = InputStreamReader(resourceAsStream!!)
    writer.write(reader.readText())
}
